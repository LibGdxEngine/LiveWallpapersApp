...
class Boids {
	...
	float mBoxMinX = 1000, mBoxMaxX = 0, mBoxMinY = 1000, mBoxMaxY = 0;
	int mStates = 4;
	...
	public void init() {
		...
		for (int i = 0; i < mTotal; i++) {
			...
			while (0 == mBoidList[i].mVelocity.x)
				mBoidList[i].mVelocity.x = (int)(2 * mMaxBoidSpeed * Math.random()) - mMaxBoidSpeed;
			
			while (0 == mBoidList[i].mVelocity.y)
				mBoidList[i].mVelocity.y = (int)(2 * mMaxBoidSpeed * Math.random()) - mMaxBoidSpeed;
		}
	}
	...
	public void moveToNext() {	
		for (int i = 0; i < mTotal; i++) {
			...
			ruleBoundPosition(i, (float)(0.7f * mMaxBoidSpeed * Math.random() + 0.3f * mMaxBoidSpeed));
	...
	private void getBoundingBox() {
		...
		for (int i = 0; i < mTotal; i++) {
			if (mBoidList[i].mPosition.x < mBoxMinX) mBoxMinX = mBoidList[i].mPosition.x;
			if (mBoidList[i].mPosition.x > mBoxMaxX) mBoxMaxX = mBoidList[i].mPosition.x;
			if (mBoidList[i].mPosition.y < mBoxMinY) mBoxMinY = mBoidList[i].mPosition.y;
			if (mBoidList[i].mPosition.y > mBoxMaxY) mBoxMaxY = mBoidList[i].mPosition.y;
		}
	}
	...
	public void setTargetPlace(float x, float y, float f) {    	
		...
		// If the touched point is within the boids, then disperse them away
		// if it is outside the boids, then make them follow it
		getBoundingBox();
		if (x >= mBoxMinX && x <= mBoxMaxX && y >= mBoxMinY && y <= mBoxMaxY) {
			mPlaceFactor = -f;
			mCount  = 0;
			mMaxCount = 50;
		} else {		
			mPlaceFactor = f;
			mCount  = 0;
			mMaxCount = (int)(Math.abs((mBoxMinX + mBoxMaxX) / 2.0f - x) + 
					Math.abs((mBoxMinY + mBoxMaxY) / 2.0f - y));
		}
	}
	
	public void setTargetNone() {
		// if direction is "following", then stop it
		if (mPlaceFactor > 0) {
			mCount = mMaxCount - 10;
		}
	}
	...
	private Velocity ruleKeepSmallDistance(int id, float dist) {
		...
		for (int i = 0; i < mTotal; i++) {
			if (i != id) {
				if (Math.abs(mBoidList[id].mPosition.x -  mBoidList[i].mPosition.x) + 
						Math.abs(mBoidList[id].mPosition.y -  mBoidList[i].mPosition.y) < dist) {
	...
	private void ruleLimitVelocity(int id, float vmax) {
		...
		float vv = (float)Math.sqrt(mBoidList[id].mVelocity.x * mBoidList[id].mVelocity.x + 
				mBoidList[id].mVelocity.y * mBoidList[id].mVelocity.y);
		
		if (vv > vmax) {
			mBoidList[id].mVelocity.x = (mBoidList[id].mVelocity.x / vv) * vmax;
			mBoidList[id].mVelocity.y = (mBoidList[id].mVelocity.y / vv) * vmax;
		}
	}
	...
	// TODO: scatter the flock b negating 3 above rules
	
	// TODO: add mPerching status when below the threshhold height
}
